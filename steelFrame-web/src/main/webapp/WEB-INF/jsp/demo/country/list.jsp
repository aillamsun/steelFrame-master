...
<div class="wrapper">
    <div class="middle">
        ...
        <form action="${ctx}/demo/country/list" method="post">
        ...
        <c:if test="${page!=null}">
            ...
            <table class="gridtable" style="width:100%;">
                <thead>
                <tr>
                    <th colspan="4">查询结果 - [<a href="${ctx}/demo/country/view">新增国家(地区)</a>]</th>
                </tr>
                <tr>
                    <th>ID</th>
                    <th>国家(地区)名</th>
                ...
                <c:forEach items="${pageInfo.list}" var="country">
                    <tr>
                        ...
                        <td style="text-align:center;">[<a
                                href="${ctx}/demo/country/view?id=${country.id}">修改</a>] -
                            [<a href="${ctx}/demo/country/delete?id=${country.id}">删除</a>]
                        </td>
                    </tr>
                </c:forEach>
                </tbody>
            ...
            <table class="gridtable" style="width:100%;text-align: center;">
                <tr>
                    <c:if test="${pageInfo.hasPreviousPage}">
                        <td>
                            <a href="${ctx}/demo/country/list?page=${pageInfo.prePage}&rows=${pageInfo.pageSize}&countryname=${queryParam.countryname}&countrycode=${queryParam.countrycode}">前一页</a>
                        </td>
                    </c:if>
                    <c:forEach items="${pageInfo.navigatepageNums}" var="nav">
                        ...
                        <c:if test="${nav != pageInfo.pageNum}">
                            <td>
                                <a href="${ctx}/demo/country/list?page=${nav}&rows=${pageInfo.pageSize}&countryname=${queryParam.countryname}&countrycode=${queryParam.countrycode}">${nav}</a>
                            </td>
                        </c:if>
                    </c:forEach>
                    <c:if test="${pageInfo.hasNextPage}">
                        <td>
                            <a href="${ctx}/demo/country/list?page=${pageInfo.nextPage}&rows=${pageInfo.pageSize}&countryname=${queryParam.countryname}&countrycode=${queryParam.countrycode}">下一页</a>
                        </td>
                    </c:if>
                </tr>
            </table>
